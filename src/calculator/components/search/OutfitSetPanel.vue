      ...
      <div v-for="tier in tiers" :key="tier.level" class="mb-2 last:mb-0">
        ...
        <div v-if="expandedTiers.has(tier.level)" class="ml-4 space-y-1.5">
          ...
          <div v-for="set in tier.sets" :key="setKey(set)">
            ...
            <!-- Role buttons for this set -->
            <div v-if="selectedSet === set" class="mt-1 ml-4 flex flex-wrap gap-1">
              <button
                v-for="(itemIds, role) in set.roles"
                :key="role"
                type="button"
                class="inline-flex items-center gap-1 rounded-md border border-[#38364A] bg-[#2D2C34] px-2 py-0.5 text-xs text-[#9B96AD] transition-colors hover:border-[#B4A5C8]/40 hover:bg-[#3B3A47] hover:text-[#EDE9F7]"
                @click.stop="addRole(itemIds)"
              >
                {{ t(`outfitSets.roles.${role}`, role) }}
                <span class="text-[#6B677A]">{{ itemIds.length }}</span>
              </button>
            </div>
          </div>
        </div>
